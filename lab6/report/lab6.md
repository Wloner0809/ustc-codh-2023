...
> **分支预测这部分没去找助教检查，因为在补数理逻辑()，而且结果也没有实现的很好。这部分加不加分没那么重要，主要是我在写完cache之前就把这部分写了个大概，所以想把它做完整点。**

* `FSM`

```mermaid
graph LR;
	A(STRONG_TAKEN) --hit--> A(STRONG_TAKEN)
	A(STRONG_TAKEN) --miss--> B(WEAK_TAKEN)
	B(WEAK_TAKEN) --hit--> A(STRONG_TAKEN)
	B(WEAK_TAKEN) --miss--> C(WEAK_NOT_TAKEN)
	C(WEAK_NOT_TAKEN) --hit--> B(WEAK_TAKEN)
	C(WEAK_NOT_TAKEN) --miss--> D(STRONG_NOT_TAKEN)
	D(STRONG_NOT_TAKEN) --hit--> C(WEAK_NOT_TAKEN)
	D(STRONG_NOT_TAKEN) --miss--> D(STRONG_NOT_TAKEN)
```

> **FSM如上图所示**
>
> 从左到右的四个状态依次代表着**一定跳、可能跳、可能不跳、一定不跳(当然实际上不是这个意思，只是描述一下预测跳不跳转的程度)**
...
(
    ...
    input id_ex_branch,         //ex阶段是否为分支指令
    input branch_taken,         
    input id_ex_predict_taken,  //ex阶段的predict_taken信号
    input stall,                //停顿流水线信号
    output reg [1:0] pc_sel,    //输出pc选择信号
    output reg predict_taken,       
    output predict_fail,   
    output reg [31:0] predicted_pc  //输出预测的pc
);
    ...
    wire [19:0] pc_tag;
    wire [4:0] pc_index;
    wire [19:0] id_ex_pc_tag;
    wire [4:0] id_ex_pc_index;
    assign {pc_tag, pc_index} = pc[31:7];
    assign {id_ex_pc_tag, id_ex_pc_index} = id_ex_pc[31:7];

    //buffer大小为32
    reg valid [0:31];
    reg [1:0] state [0:31];
    reg [19:0] tag [0:31];
    reg [31:0] target_pc [0:31];
    
    //四个状态的定义
    //根据第一位是0还是1来判断是否taken
    ...
    //判断是否命中
    always @(*) begin
        if((pc_tag == tag[pc_index]) & valid[pc_index] & (~state[pc_index][1])) begin
            predict_taken = 1;
            predicted_pc = target_pc[pc_index];
        end
        else begin
            predict_taken = 0;
            predicted_pc = 0;
    ...
    //失败的两种情况
    reg fail1;  //该变量代表预测跳转但实际没跳转
    reg fail2;  //该变量代表预测不跳转但实际跳转
    always @(*) begin
        if(id_ex_predict_taken & ~branch_taken) begin
            fail1 = 1;
            fail2 = 0;
        end
        else if(~id_ex_predict_taken & branch_taken) begin
            fail1 = 0;
            fail2 = 1;
        end
        else begin
    ...
    assign predict_fail = fail1 | fail2;
    ...
    always @(posedge clk or negedge rstn) begin
        if(!rstn) begin
            //初始化变量
            //将state变量初始化为WEAK_NOT_TAKEN
            for (i = 0; i < 32 ; i = i + 1) begin
                state[i] <= WEAK_NOT_TAKEN;
                valid[i] <= 0;
                tag[i] <= 0;
                target_pc[i] <= 0;
            end
        end
        else if(~stall & id_ex_branch) begin
            if((tag[id_ex_pc_index] == id_ex_pc_tag) && valid[id_ex_pc_index]) begin
                if(state[id_ex_pc_index] == STRONG_TAKEN)
                    state[id_ex_pc_index] <= predict_fail ? WEAK_TAKEN : STRONG_TAKEN;
                else if(state[id_ex_pc_index] == WEAK_TAKEN) 
                    state[id_ex_pc_index] <= predict_fail ? WEAK_NOT_TAKEN : STRONG_TAKEN;
                else if(state[id_ex_pc_index] == WEAK_NOT_TAKEN)
                    state[id_ex_pc_index] <= predict_fail ? WEAK_TAKEN : STRONG_NOT_TAKEN;
                else
                    state[id_ex_pc_index] <= predict_fail ? WEAK_NOT_TAKEN : STRONG_NOT_TAKEN;
            end
            else begin
                valid[id_ex_pc_index] <= 1;
                tag[id_ex_pc_index] <= id_ex_pc_tag;
                target_pc[id_ex_pc_index] <= branch_target;
                state[id_ex_pc_index] <= WEAK_NOT_TAKEN;
        end
        end
    end
...
> **pc_sel输出pc的选择信号，这部分需要在cpu中作一定的修改**
>
> **整体的设计类似cache，不再赘述**
...
|         预测          |          不预测          |
| :-------------------: | :----------------------: |
| ![](pics/predict.png) | ![](pics/no_predict.png) |
| ![](pics/predict.JPG) |  ![](pics/predict.JPG)   |

> 我是用LFSR生成7个随机数，然后进行排序
>
> **结果如上图的原因可能是：汇编生成的数据不太好/排序算法实现中分支指令的应用不好，让分支预测器在预测跳转而实际不跳的这种情况下损失一些周期，正好中和了预测成功节省下来的周期**